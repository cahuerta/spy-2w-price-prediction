...
def _run_full_math_engine(ticker, horizon, pca_target, theta,
    ...
    if res_df is None or len(res_df) == 0:
        ...
        hist = HistoricalStats(
            ...
            rmse_mean     = float(np.sqrt(mean_squared_error(
                [0]*len(res_df), res_df["rmse"].values
            ))) if len(res_df) > 0 else None,
    ...
    w = float(np.clip(hist.hit_rate_mean, 0.4, 0.6)) if hist.hit_rate_mean is not None else alpha
...
def run_predictor_h10(
    ...
    if DARWIN_PREDICTOR:
        try:
            ...
            # H10 NO usa feature_override — su identidad matemática (Hurst+kNN) es fija
        except Exception:
            pass
    # ─────────────────────────────────────────────────
