...
# PREDICTOR H3
# ======================================================
def run_predictor_h3(ticker: str, override_genome=None):
    raw = get_price_history(ticker=ticker, period="2y", interval="1d")

    # ── DARWIN: cargar genoma activo ──────────────────
    _alpha_from_genome = None  # [AUD-P11] antes: _alpha = ALPHA_H3 (fijo, nunca recalibrado)
    ...
    # vía RidgeCV (walk-forward) en vez de usar ALPHA_H3 fijo —
    ...
    if _alpha_from_genome is not None:
        ...
        _alpha = select_alpha_ridge_cv(X, y, dynamic_pca, fallback_alpha=ALPHA_H3)
