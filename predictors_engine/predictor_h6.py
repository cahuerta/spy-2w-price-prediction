...
# [DETREND] Mismo mecanismo aplicado a H1-H5 (auditoría 2026-08-24): se
...
DRIFT_WINDOW      = int(os.getenv("H6_DRIFT_WINDOW", "60"))
DRIFT_MIN_PERIODS = int(os.getenv("H6_DRIFT_MIN_PERIODS", "30"))
...
# PREDICTOR H6
# ======================================================
def run_predictor_h6(ticker: str, override_genome=None):
    raw = get_price_history(ticker=ticker, period="2y", interval="1d")

    # ── DARWIN: cargar genoma activo ──────────────────
    _alpha_from_genome = None  # [AUD-P11] antes: _alpha = ALPHA_H6 (fijo, nunca recalibrado)
    ...
    # ya corregido en H1-H5: _drift_local es la media DIARIA, y_fwd es
    # el retorno acumulado a HORIZON=6 días).
    ...
    # vía RidgeCV (walk-forward) en vez de usar ALPHA_H6 fijo —
    ...
    if _alpha_from_genome is not None:
        ...
        _alpha = select_alpha_ridge_cv(X, y, dynamic_pca, fallback_alpha=ALPHA_H6)
...
if __name__ == "__main__":
    ...
    if result:
        ...
        # [FIX] _decay es local a run_predictor_h6() — no existe en este scope
        # (mismo bug identificado y corregido en predictor_h1-h5.py). Se lee
        # desde el propio resultado ya guardado en el JSON.
        print(
            f"✅ H6 | ${result['price_today']:,.2f} → ${result['price_pred']:,.2f} | "
